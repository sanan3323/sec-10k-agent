...
**Status:** Phase 6 (agent: router, decomposition, multi-hop retrieval + the XBRL tool, synthesis, LLM-judge citation verification with one bounded retry, cited-answer finalization) is implemented and unit-tested end-to-end via `sec10k agent`; not yet run against the live corpus (see Known limitations). Phase 5 (hybrid retrieval) is implemented and measured, kept opt-in. `sec10k ask`/`sec10k eval --hybrid` add BM25 lexical fusion (RRF) and cross-encoder reranking on top of dense retrieval. Measured against the Phase 4 dense-only baseline on the 26-item seed set, the result is mixed, not a clean win: faithfulness rose (0.89 → 0.94) but correctness fell (0.73 → 0.62), driven almost entirely by the temporal bucket (0.67 → 0.37) — the cross-encoder reranks each chunk by topical relevance alone, with no fiscal-year diversity awareness, so multi-year comparison questions can end up served several same-year chunks instead of one-per-year coverage. Both baselines are committed under `data/eval/baselines/` for comparison. **Hybrid stays opt-in (`--dense-only` is still the default)** pending diversity-aware selection — a natural fit for the router's `is_temporal` detection. Phase 4 (eval harness), Phase 3 (RAG MVP), Phase 2 (pgvector retrieval), and Phase 1 (ingestion) are done. Phase 1c (XBRL) is partial; see Known limitations.
...
| 6 | implemented, unverified live | Agent: hand-rolled state machine (ADR-002), router, decomposition, multi-hop retrieval + XBRL tool, LLM-judge citation verification, finalize | `sec10k agent` with `--trace`; not yet run against the live corpus or scored on a multi-hop eval bucket |
...
- **Phase 6 (agent) is unit-tested only, not yet run live.** All nodes and the orchestrator pass their tests with fakes for the retriever/generator, but `sec10k agent` hasn't been exercised against the real pgvector corpus + a live generator, and there's no multi-hop eval bucket yet to score it against.
